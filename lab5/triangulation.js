...
var removed_triangles= {};
	//triangles to be removed before returning
var tree_structure = {};
	//tree[triangle_id] = {"sons":["sub triangles","of","triangle_id"], "fathers":father_triangle}
var DCEL = {};
...
function findTriangleTree(point,insideOf,prof){
	var sons_set = tree_structure[insideOf].sons;
	
	if(sons_set.size >0){
		
		for(const son of sons_set){
			//console.log("looking inside subtriangle "+ (son));
			var t = triangles[son].edges;
			if(trianglePoint(point,t)){ //if this point is inide the sub triangle
				//console.log("im inside of "+ son +" a profundidad " + prof);
				return findTriangleTree(point,son,prof+1)
			}
		}
	}
	else {
		console.log(triangles[insideOf])
		//console.log(triangles[insideOf].edges.map(m => DCEL[m]))
		return insideOf;
	}
}
...
function flip(edge){
	...
	var new_edge = Object.keys(DCEL).length;
	var new_prima = Object.keys(DCEL).length+1;

	var T_izquierda = {'edges':[DCEL[DCEL[prima].eN].eN,DCEL[edge].eN,new_edge]};
	var T_derecha = {'edges':[DCEL[DCEL[edge].eN].eN,DCEL[prima].eN,new_prima]};

	//console.log(T_izquierda,T_derecha);
	
	var new_FR = DCEL[edge].fR + "0";
	var new_FL = DCEL[edge].fL + "0";

	triangles[new_FL] = T_derecha;
	triangles[new_FR] = T_izquierda;


	removed_triangles.add(DCEL[edge].fR);
	removed_triangles.add(DCEL[edge].fL);
	...
	DCEL[new_edge] = {"vB":inserted_point, "vE":other_point, "fL":new_FL, "fR":new_FR,"eN":DCEL[DCEL[prima].eN].eN, "ePrima":new_prima};
	DCEL[new_prima] = {"vB":other_point, "vE":inserted_point, "fL":new_FR, "fR":new_FL, "eN":DCEL[edge_copy.eN].eN, "ePrima":new_edge};

	//updating edges info
	//primer triangulo
	var next = DCEL[new_edge].eN
	DCEL[next].eN = edge_copy.eN;
	DCEL[next].fR = new_FR;
	DCEL[DCEL[next].ePrima].fL = new_FR;

	next = DCEL[next].eN;
	DCEL[next].eN = new_edge;
	DCEL[next].fR = new_FR;
	DCEL[DCEL[next].ePrima].fL = new_FR;

	//seguno triangulo
	next = DCEL[new_prima].eN
	DCEL[next].eN = prima_copy.eN;
	DCEL[next].fR = new_FL;
	DCEL[DCEL[next].ePrima].fL = new_FL;

	next = DCEL[next].eN;
	DCEL[next].eN = new_prima;
	DCEL[next].fR = new_FL;
	DCEL[DCEL[next].ePrima].fL = new_FL;

	//updating vertices info
	vertices[inserted_point].edges.push(new_prima);
	vertices[other_point].edges.push(new_edge);
	...
	//updating tree_structure
	//console.log("flipped edge " + edge+ "/" + prima + " into "+new_edge+"/"+ new_prima)
	//console.log("turned faces " + DCEL[edge].fR+ "/" + DCEL[edge].fL + " into "+new_FR+"/"+ new_FL)
	tree_structure[new_FR] = {"sons" : new Set(),"fathers":new Set([DCEL[edge].fR,DCEL[edge].fL])};
	tree_structure[new_FL] = {"sons" : new Set(),"fathers":new Set([DCEL[edge].fR,DCEL[edge].fL])};
	tree_structure[DCEL[edge].fR].sons.add(new_FR);	tree_structure[DCEL[edge].fR].sons.add(new_FL);
	tree_structure[DCEL[edge].fL].sons.add(new_FR);	tree_structure[DCEL[edge].fL].sons.add(new_FL);
	return(new_prima);
...
function computeTriangulation(points) {
	...
	tree_structure["T"] = {"sons" : new Set(),"fathers":"Null"};
	removed_triangles.add("ext")
	...
	for (i=0;i<points.length-3;i++){
		...
		v = points[i];
		console.log(`adding point: `);console.log(v)
		...
		if(i==0){
			...
			insideOf = findTriangleTree(v,"T",0);
		...
		var startTime = performance.now();
		//console.log("breaking " + insideOf + " into three pieces")
		...
		for(e = 0; e<OGtri.edges.length; ++e){
			...
			tree_structure[insideOf].sons.add(insideOf+e);
			tree_structure[insideOf+e] =  {"sons" :new Set(),"fathers":new Set([insideOf])};
			...
			while(del_edges.length>0){
				...
				if(tri != "ext"){
					...
					if (circleOrientationTest(points[i],vert_incident_triangle)<0) {
						...
						if(flips.has(DCEL[DCEL[edg_in].ePrima].eN)){
							//console.log("hay caramba")
							continue;
						}
						del_edges.push(edg_in);
						//console.log("swap");
						//console.log(`edge to flip -> ${DCEL[DCEL[edg_in].ePrima].eN}`);
						var ret = flip(DCEL[DCEL[edg_in].ePrima].eN);
						del_edges.push(ret);
						flips.add(ret);
					}
				}
	...
	console.log("removed_triangles");console.log(removed_triangles);
	console.log("tree_struct");console.log(tree_structure);
	...
	for (let x in triangles) {
		if(!(removed_triangles.has(x))){
			...
			tri_copy[x] = triangles[x];
		}
		++i;
	}
	console.log(tri_copy);
	return outputTriangles;
}


function prune(boundaries){

	for(i=1;i <= 3;++i){
		var point = points.length-i;
		for(const e of vertices[point].edges){
			removed_triangles.add(DCEL[e].fL)
			removed_triangles.add(DCEL[e].fR)
		}
	}
	...
	for (const bound of boundaries){
		if(bound.length ==1){
			var q = bound[0];
			for(const e of vertices[q].edges){
				removed_triangles.add(DCEL[e].fL)
				removed_triangles.add(DCEL[e].fR)
			}
		}
		else{
			var bounding_points = new Set(bound);
			for(i=0;i<bound.length+1;++i){
				var p = bound[(i+bound.length-1)%bound.length];
				var q = bound[i%bound.length];
				var r = bound[(i+1)%bound.length];
				console.log(p,q,r)
			
				for(const e of vertices[q].edges){
					var point =DCEL[e].vB;
					console.log("Checking vertix " + point)
					if(point != p && point !=r && outsideBoundary(p,q,r,point)){
						console.log("vertix " + DCEL[e].vB+" is out of bounds, removing face " + DCEL[e].fR )
						removed_triangles.add(DCEL[e].fR)
						removed_triangles.add(DCEL[e].fL)
	...
	var outputTriangles = new Array;
	for (let x in tri_copy) {
		if(!(removed_triangles.has(x))){
			outputTriangles.push(tri_copy[x].edges.map(m => DCEL[m].vE));
...
// ------------------------------------------------------------------------
function outsideBoundary(p,q,r,point){
	var d1 = determinant2x2(vertices[p],vertices[q],vertices[point]);
	var d2 = determinant2x2(vertices[q],vertices[r],vertices[point]);
